...
def main():
    ...
    for next in index[1:]:
        if prev[0] == next[0]:
            if (next[1] - prev[1]) == args.winsize:
                if len(cnv_cand) > 0:
                    if cnv_cand[-1] != prev:
                        cnv_cand.append(prev)
                else:
                    cnv_cand.append(prev)
                cnv_cand.append(next)
        prev = next
    ...
    ax.set_xticklabels([extract_chrnum(chr) for chr in chrs])
    #ax.set_xlabel('Chromosome', fontsize='xx-large', fontweight='bold')
    ax.set_ylabel('Coverage (base)', fontsize='xx-large', fontweight='bold')
    fig.tight_layout()
